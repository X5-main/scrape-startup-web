...
AVELIN API is compatible with [Shakespeare](https://shakespeare.dev) — an open-source app builder that runs entirely in your browser with no backend server required.

Shakespeare lets you describe an application in natural language and watch it materialize in real-time — all without spinning up a server, installing a CLI, or configuring a development environment. The entire build and preview loop happens client-side in your browser tab. This makes it uniquely accessible: share a link, open it on a Chromebook, use it on a locked-down corporate laptop — anywhere you have a browser, you have a full app builder. For prototyping, demos, and quick internal tools, Shakespeare collapses the path from idea to working app to essentially zero setup.

> **Why AVELIN + Shakespeare?** Access AVELIN's frontier coding models with 1M-token context windows, automatic failover, and ~100 tps throughput through a single OpenAI-compatible endpoint.
...
### Step 2: Install Shakespeare

Refer to [Shakespeare documentation](https://shakespeare.dev) for installation instructions.
...
- [Shakespeare Documentation](https://shakespeare.dev) — Official docs
