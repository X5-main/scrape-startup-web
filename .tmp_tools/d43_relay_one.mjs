// Browser-relay vision half (single page): open URL as background tab in David's
// Chrome via relay 9224, settle, sweep lazy images, capture top viewport band,
// dump DOM facts, close tab. Per-step hard timeouts.
// Usage: bun d43_relay_one.mjs <url> <out.png>
const [, , url, out] = process.argv;
if (!url || !out) { console.error("usage: d43_relay_one.mjs <url> <out.png>"); process.exit(1); }
...
try {
  ...
  const pmsg = (m, p = {}) => withT(new Promise((res, rej) => { const id = ++pidc; ppend.set(id, { res, rej }); bws.send(JSON.stringify({ id, method: m, params: p, sessionId: sid })); }), 20000, m);
  ...
  console.log("RELAY_STATE", url, JSON.stringify(state));
  ...
  if (cap.result?.data) { await Bun.write(out, Buffer.from(cap.result.data, "base64")); console.log("WROTE", out, state.ih + "px"); }
  else console.log("NO-DATA", url);
} catch (e) {
