...
## D55 close — Micro1 (micro1.ai)

- **Lane/target:** TC funding wire fresh lane (leadprysm aug/jul pools exhausted since D52, byte-identical) → **micro1.ai** ("Micro1 — Data lab to train frontier models & evaluate agents") — AI training-data startup: human-expert RL gyms, synthetic-data generation (scale-ai class); founder Ali Ansari. **Funding gate:** TechCrunch Aug 20 2026 "$500M gross run rate" (https://techcrunch.com/2026/08/20/ai-data-startup-micro1-reaches-500m-gross-run-rate-amid-ai-training-boom/) — 4-yr-old, $100M→$500M gross ARR in ~8 months (net ~$150–200M, 60–70% retention), Series A @ $500M val Sept 2025 (≤24-mo ✓); possible fresh raise at a higher valuation unconfirmed. Comp: Mercor $2B gross, Handshake $1B.
- **Screening:** relay viewport-band judge (David's real Chrome, 1512×716, hero-focused) — micro1 **7/10** → round target elected (non-binding screening per D38); stability.ai 5 = backup only.
- **Mirror:** `python3 mirror_site.py https://www.micro1.ai/ micro1.ai --depth 2` → **1638 files / 111 MB / 349 HTML** (Webflow hash dirs; forum CMS + benchmark subsection + research; 372s). Static Webflow → byte-faithful rel-rewrite serve, no RSC/dpl closure surgery, no `.origin` file needed (same absence as rillet/stability).
- **SCRIPT-BODY CORRUPTION CLASS (fix this round, both tools):** residual sweep flagged served inline JS containing `/benchmark/crosby-micro1-redlinebench/${b(s.src)}` — the serve-time `_restore_ref` pass (`posixpath.join(pagedir, url)`) was prefixing the browser-runtime template literal `${b(s.src)}` (Webflow cache-bust JS construct, NOT a static ref). Root cause: `_MASK_SCRIPT_BODIES` gate was `bool(re.search(r"lyzr\.ai", ORIGIN))` → false for micro1 (no `.origin` file). Fix: mask when `os.path.basename(DOCROOT).startswith("micro1")` too — for static-Webflow mirrors the on-disk script bodies ARE live bytes (mirror never rewrites scripts), so masking is strictly byte-faithful. Same masking wrote into `residual_sweep.py` (script/style bodies blanked before attr extraction). Verified: served crosby page `src="[^"]*\${[^"]*"` → 0 hits; served-vs-disk diff 84→82 changed lines, all legit markup attr restores.
- **Residual sweep closed:** `pages=349 refs=1698 residual_uniq=7`, ALL parity-justified: (a) 5× Webflow comma-in-filename srcset family (`6a2dbc…Candidate is hired, and works 20 hours (2)-p-{500,800,1080,1600}.webp` + base) — srcset parser splits at the literal comma inside the filename; the valid `img src` comma-named file exists on disk/serves 200, the srcset candidates fail identically on LIVE (mirror faithful); (b) `/research/assets/thumb.jpg` — 404 on live; (c) `/plugins/Basic/assets/placeholder.60f9b1840c.svg` — Webflow `w-dyn-bind-empty` CMS-empty logo slot in 292 forum pages' real `<img>` markup.
- **Advisory-driven re-verification (S3 AccessDenied ≠ 404):** curl CDN direct → AccessDenied XML is NOT the browser path — the relative `src="../../plugins/..."` resolves against **www.micro1.ai**, and the real-browser relay navigation probe (d55_probe_nav.mjs, Page.navigate not fetch()) returned Webflow's branded 404 page ("Page not found | micro1") for both the exact resolution path and the CDN alt; replica serves 404 → **status parity 404≡404**, asset genuinely absent on live. Nothing vendored (no 403-XML or 404-HTML stand-ins — parity-over-fake-fixes rule). Navigation-based relay probing is the authoritative real-browser method; `fetch()` from null-origin about:blank CORS-fails.
- **Vision closure (relay full-page, flushed-identical pixels 1512×5241 both):** live **5/10**, replica **5/10** — identical critique (void-heavy dark hero, tiny low-contrast footer): identical judge evidence proves visual parity, but binding ≥7 reproducible NOT achieved on live either (band 7 = screening-only; D53 lesson: the band, not the max, carries reproducibility). **D38 binding gate UNMET, recorded** — D54-precedent close class: fidelity-verified, gate-unmet, comparative-best.
- **DOM facts (relay full-page):** title verbatim identical ("Data lab to train frontier models & evaluate agents | micro1"), h1 verbatim identical, scrollHeight **5241==5241**, docWidth 1512==1512; innerText 1962 vs 1630 (live renders more Webflow runtime JS-bound text; structural DOM parity high).
- **Lessons for D56:** (1) A viewport-band 7 is screening-only; the binding gate is the full-page flushed-pixel re-judge — a band-7 target can bind-fail at 5/5 with the judge calling whole-page voids. (2) Webflow comma-in-filename srcset entries are a permanently-faithful residual class — the srcset parser splits at the comma; never "fix" them. (3) CDN AccessDenied XML is not the browser-facing error: resolve relative refs exactly as the browser does (site host, not CDN host) before classifying 404-on-live. (4) `_MASK_SCRIPT_BODIES` site-gating by `ORIGIN` alone silently fails sites without `.origin` files — gate on DOCROOT basename too, and give the sweeper the same mask so template-literal "residuals" never re-appear.

---
### D55 addendum — design_signal evidence (advisory: capture actual signal outputs, not just scores)

Recorded per candidate via `python3 .tmp_tools/capture_signal.py <name> <path-or-url>` → `.tmp_tools/signals/<name>.json` (full stats + score + verdict + boilerplate). Prior to D55-addendum, only the vision scores were persisted; from D56 on, the signal JSON is part of round evidence BEFORE any mirror (also the elected candidate's mirror index.html, so the snapshot is byte-faithful).

- **d55_micro1** (source `micro1.ai/index.html`, the committed mirror root — byte-faithful snapshot): **5.2/10 → JUDGE** | html 147487B, text 3116 chars, h1 1 / h2 7 / h3 3, nav 3 footer 1 imgs 19, logo-wall 3, fonts: display `outfit` (0 links, 0 @font-face), dark_body True (bg-lum 0.315), animation gsap/lottie/swiper, **boilerplate: none**. Note: 5.2 is pre-visual; actual D55 binding vision pair logged live 5 / replica 5. No template marker in the home HTML — the Webflow runtime classes live in deeper/hashed pages, not index.
- **d55-followup_stability_2026-08-26** (source `https://stability.ai/` live fetch run **2026-08-26 — dated follow-up observation, NOT D55-era evidence**: stability was never mirrored or snapshotted during D55, so today's HTML may have drifted from the page the D55 judge saw; the D55-era record for stability remains the vision score 5 only): **6.9/10 → JUDGE** | html 525381B, text 6351 chars, h1 0 / h2 13 / h3 1, nav 4 footer 1 imgs 58, canvas 4, logo-wall 5, fonts: 14 @font-face, display `archivo`/`figtree`, dark False, animation three.js, **boilerplate: ['squarespace']**. **Material qualification (binding for ANY future stability round):** the −0.35 template penalty is already inside the 6.9; without it the pre-filter would be ~7.25. The D55-era record for Stability is the screen-time vision score 5 only — no Stability-specific critique was captured (the "void-heavy dark hero, tiny low-contrast footer" critique in the D55 close belongs to Micro1's elected full-page pair, not Stability), and the 2026-08-26 HTML may differ from what the judge saw. The Squarespace marker is a qualification for FUTURE rounds only: if stability.ai ever wins on visuals, this signal record caps the claim at template-credit (raw ~7.25), not bespoke design. Same rule for any candidate flagged `w-webflow|wix|squarespace|elementor|...` by the BOILERPLATE regex.
- **Rule (D56+):** run capture_signal.py on every candidate AT SCREENING TIME (same session — contemporaneous, not back-filled) and on the elected mirror's index.html post-mirror; commit the JSONs with the round. Any signal run after the round closes, against a non-snapshotted source, is a **dated follow-up observation** — file it with the run date in the name and never present it as round evidence. A non-empty `boilerplate` field goes verbatim into the round README row as a qualification.

---

## D56 close — Airbound (airbound.com)



- **Lane/target:** TC funding-wire fresh lane (leadprysm aug/jul pools still exhausted/byte-identical) → **airbound.com** ("Airbound — INDIA'S MOST AUDACIOUS HARDWARE COMPANY" — autonomous rocket-like delivery drones for India, "a new class of aircraft, light enough to launch anywhere and smart enough to fly itself"). **Funding gate:** TechCrunch Aug 24 2026 (Jagmeet Singh, [TC story](https://techcrunch.com/2026/08/24/indias-airbound-bags-37m-to-take-on-trucks-with-rocket-like-drones/), published 2026-08-25T03:30:00Z) — **$37M Series A**, backers Greenoaks / DoorDash / Lachy Groom / Lightspeed India / Humba; funding_date 2026-08-24 ≤ 2026-08-25 gate ✓.
- **Pool sweep (D56, TC wire):** airbound elected live **8/10** first-screen (non-binding screening per D55 lesson #1); keenable 6/10 (Framer); generalintuition 4/10; stability **6.9/10 → JUDGE** (`.tmp_tools/signals/d56_stability.json`, captured_at 2026-08-25T21:53:40Z — contemporaneous D56 screening capture, same 21:53Z sweep as airbound; boilerplate `['squarespace']`; the D55-era vision 5 remains the historical record only); gatik/inherent/letara skipped; navi 403 (unreachable); luffu no funding. All screening-time signals committed (`.tmp_tools/signals/d56_*.json`).
- **Mirror:** `python3 mirror_site.py` → **247 files / 35 MB / 8 HTML** — Webflow + cross-host asset prefixes vendored under relative host-path dirs: `68c2a094…` (Webflow CDN), `npm/`+`gsap/` (jsdelivr), `duganoymz/` (res.cloudinary), `ajax/` (googleapis), `g0lnomhfn3mg…/index.html` (GTM container, 524 KB). Serve `serve_replica.py 8970` → `/index.html` 200. **Residual sweep closed**: `residual_uniq=2`, both reconciled as runtime JS string-concat fragments inside the stored GTM container (identical on live — never "fix"); cldPlayer.js (52,760 B, HTTP 200) vendored.
- **design_signal (both committed with round):** **d56_airbound** (live `https://www.airbound.com/`, captured_at 2026-08-25T21:53Z) **4.9/10 → JUDGE** | html 102436B, text 4924 chars, h1 4 / h2 3 / h3 10, nav 1 footer 0 imgs 30, logo-wall 1, fonts: display `inherit` (1 @font-face, 0 links), dark False, animation gsap/swiper, **boilerplate: []** verbatim. **d56_airbound_mirror** (post-mirror source `airbound.com/index.html`, captured_at 2026-08-26T10:11Z) **4.9/10 → JUDGE** | html 97091B, text 4924 == 4924, rest of profile identical to live — byte-faithful mirror confirmed at signal level; **boilerplate: []**.
- **Pixel parity (relay band capture, DPR-2 3024×1432 ×8 bands, page 5388 CSS px):** raw 8-band pair **6/8 bands byte-identical** (bands 1,4–7); band 0 (animated hero) mean_abs 0.828, 0.6% channels >8; band 2 0.252, band 3 0.055, all 0.00% >8; whole-page mean_abs **0.151, 0.08% channels >8** — flush parity essentially total, divergence confined to hero animation frames.
- **TOOLING BUG FIX (this round — corrupt judge input, affects prior D56 full-page scores):** `stitch_bands.py` write_png prepended filter type 0 (None) but stored **average-filtered bytes** — decoded images carried row-wise prediction-residual garbage that looked like scanline/noise bands; round-trip write→read mean 122.7 vs 0.000 after fix (store raw rows unfiltered under type 0). The prior full-page judge inputs (`/tmp/d56_*_s1.png`) and their 4/6/6 replica / 7/6 live scores were artifacts of this corruption, not the replica — and the "(4,6,6)" scanline complaints are now explained. Re-derived judge inputs: pure-stdlib 2×2 box-average (not sips — sips resampling amplified sub-8 deltas to max 255): full-page pair **mean 0.146, 0.08% >8**.
- **Binding re-judge battery (full page, same prompt, same flushed inputs):** live **8/10** → control re-judge **7/10**; replica **8/10** → control re-judge **7/10** — full-page clause of the D38 rule **MET (≥7 reproducible on identical flush pixels, both sides, 2 judges each)**.
- **Top-band (first-viewport) D38 precedent, paired crops 1512×716 from fixed stitches:** live **6/10**, replica **9/10** — **NOT both ≥7; gate recorded UNMET (strict)**. The 6-vs-9 split on a 0.6%-differing pair (hero animation frames only) shows the judge is not frame-invariant on the animated hero: the flushed live frame caught a hazy low-contrast hero moment, the replica frame a crisp one — identical DOM, parity-proven pixels, verdict flip. Election-time live 8 (different animation frame) stands as screening, not binding.
- **Conclusion (D54/D55-precedent close class):** fidelity-verified (byte-identical bands, signal-equivalent mirror, full-page reproducibly ≥7 both sides) but **D38 binding gate UNMET as recorded — top-band clause fails on live's static hero frame**, animation variance not replica defect. See README row + §D56.
- **Lessons for D57:** (1) `write_png`-style hand-rolled PNG encoders MUST round-trip-test (write→read mean == 0) before use — a filter-type/filter-body mismatch silently corrupts judged images into scanline-looking artifacts, indistinguishable from render defects. (2) sips downscale resampling amplifies sub-8 raw diffs to visible (mean 2.3, max 255) — for flush-parity judge inputs use a box-average downscaler, and only pure-stdlib tools when the .tmp_tools dir is the artifact home. (3) The full-page binding gate and the D38 top-band precedent are separate clauses: run BOTH (full-page pair + first-viewport pair from the SAME stitches) before declaring MET — an animated hero can pass the first and fail the second on frame noise alone. (4) Persist the post-mirror signal (D55 addendum rule) at the same captured_at standard as screening ones — d56_airbound_mirror confirms byte-faithfulness at signal level (text 4924 == 4924).

---

## D57 close — Ringg (ringg.ai)

- **Lane/target:** TC funding-wire fresh lane runs **2026-08-25** (leadprysm aug/jul pools still exhausted); TC wire surfaces **Ringg** in India-fintech coverage (Paytm CEO reveal, Aug 2026): **"Ringg (ringg.ai) — Conversational intelligence to 10x your business"** — AI voice agents for consumer businesses ("AI agents that deliver outcomes you want across Voice, Chat, WhatsApp and Web"), customers Cred / Flipkart / Practo / Groww / Policybazaar. **Funding gate:** $10M Series A extension led by **Peak XV**, announced 2026-08-25 ([TechCrunch](https://techcrunch.com/2026/08/25/ringg-bags-10m-to-help-companies-voice-automate/)); funding_date 2026-08-25 ≤ 2026-08-26 gate ✓; AI focus ✓.
- **Election:** pool = TC wire candidates post-sweep; design-signal screening **d57_ringg 5.7/10 → JUDGE** (captured_at 2026-08-26T10:27Z, contemporaneous); no leadprysm candidate ≥ screened overwhelm — elected on TC freshness + AI-agent relevance; D57 proceeds on the JUDGE path (vision verdict decides).
- **Mirror:** `python3 mirror_site.py` → **550 files / 70,212 KB / 204 HTML** (123 sitemap locs; **23 stale** sitemap locs verified live-404 and dropped; **33 residual sweep URLs all live-308→terminal-404 chains**, stubbed via new ORIGIN-gated generic `_ringg_redirect` hook in serve_replica.py — mirrors Next.js middleware redirects; residual 404s acceptable post-stub). Serve `serve_replica.py 8970` → `/` 200.
- **Runtime-discovered root assets (link-crawler blind spot):** Turbopack/Next.js root JS fetches assets by URL not covered by the `<link>` graph — live 200, first mirror 404. 5 such targets verified and fetched byte-exact: `_next/static/immutable/chunks/0f0bss92659v-.js` (7407 B) + `1figie2hrcyhg.js` + `1qr2z0czjs02y.js`, `c8f180f32f6b35f9/script.js` (12567 B), `fb08a4518718dc17/script.js` (2495 B). **This was the layout gap: replica page height 9505 → 9935 == live 9935 EXACT after vendoring** — same class as the D56 footer-asset gap.
- **Parity evidence (same disposable Chrome 9345 for both, DPR 1, 1791×754 × 14 bands, page 9935 CSS px both):** heights byte-equal 9935==9935; text `body.innerText` tails 700-char identical (live vs replica); whole-page stitched diff **mean_abs 1.454, 1.32% channels >8** — divergence confined to animation/overlay frames (hero mockup, cookie-consent pill, chat widget — present on BOTH sides; parity intact). Live net-errors 0; replica 0 404s, single `ERR_BLOCKED_BY_ORB` = cross-origin analytics fetch the browser refuses to READ (content delivered); non-defect.
- **design_signal (both committed with round):** **d57_ringg** (live https://www.ringg.ai/, captured_at 2026-08-26T10:27:19Z) **5.7/10 → JUDGE** | html 139267B, text 2416 chars, h1 2 / h2 5 / h3 5, nav 1 footer 0 imgs 35, logo-wall 2, fonts: display `geist-sans` (0 links, 0 @font-face), dark False, animation none, **boilerplate: []**. **d57_ringg_mirror** (post-mirror source `ringg.ai/index.html`, captured_at 2026-08-26T11:28:53Z) **5.7/10 → JUDGE** | html 122285B, text 2416 == 2416, rest of profile identical — byte-faithful mirror confirmed at signal level; **boilerplate: []**.
- **Stitch/geture notes (D57 tooling):** `d57_cap5` band-scroll driver — `scrollBehavior='auto'` + `window.scrollTo({top:y,behavior:'instant'})` + verify `scrollY===y` (retry ×5) — kills drift. Stitch by **logged ys** (`ys.json`), paste each band at its actual start y, final band cropped to exact H (naive concat duplicates the overlap — band ys overlap by design). Pure-stdlib `boxavg_downscale` (+ shared read_png/write_png, round-trip-verified) for both stitches and judge inputs; factor-2 needs even dims — 1791×9935 both odd → drop rightmost column + bottom row identically both sides (degenerate ±1 px) → 895×4967 full / 895×377 top-band judge inputs.
- **Binding D38 gate — BOTH clauses evaluated, UNMET (strict):**
  - Full-page flushed-pixel re-judge (fp prompt), three independent runs, all <7 both sides: **live 6/6/4, replica 6/5/5** (run2 = in-session control re-judge; run3 = same-input re-dispatch — ±1..2 frame variance on the animated hero, the D56 top-band lesson; every frame sub-7, verdict frame-robust).
  - Top-band (first-viewport, paired 895×377 crops): live **6/10**, replica **6/10** — <7 both sides.
  - Judge cites: "clean but flat premium," cookie-consent pill + chat widget intrude on the first screen, hero lacks motion; identical on both sides — parity confirmed, score not replica defect.
- **Conclusion (D56 precedent close class):** fidelity-verified (H 9935==9935, text-identical, 98.7% channels ≤8, signal 5.7==5.7) but **D38 binding gate UNMET (strict) — BOTH clauses fail (full-page 6/6, top-band 6/6; all four judgments reproducible <7)**. See README row + §D57.
- **Lessons for D58:** (1) The generic ORIGIN-gated `_<slug>_redirect` hook (GCP/HTML3308-style) closes 308-stub residue WITHIN serve_replica.py — no per-URL special-casing; keep the sweep discipline (every residual reconciled as terminal-404, never "fix" mid-capture). (2) Root-referenced assets (chunk/script.js at root paths) are invisible to link crawlers — runtime JS discovery via a flushed load + network log is mandatory for Next/Turbopack mirrors; page-height identity live-vs-replica is the single most sensitive mirror-fidelity meter, cheaper than pixel-diffing. (3) Judging an animated cookie pill/chat-widget page: both are in the DOM on both sides — flush both identical frames before the judge or the 0.6%-frame-noise verdict-flip lesson (D56 top-band) repeats. (4) Two independent reproduction runs pinned the 6/10 — the "±1 = judge noise" rule means 6→5 (replica fp, run1) vs 6→6 (run3) is noise; all runs <7 declares UNMET decisively.
