...
Rejected-list (26th round): Verdict Machine (SPA, uninteractable shell), BeeSafe (robots.txt returns HTML not rules), Parameter (backup only). Helper: `.tmp_tools/` sweep variant for :8924 not needed — sitemap sweep + asset URL sweep zeroed directly from the served ports.
