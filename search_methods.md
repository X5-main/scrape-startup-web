...
- **Relay tab-binding**: the browser-relay `tab` handle can bind to a stale/previous turn's tab; always verify identity inside the capture run (title/h1/bodyH/canvas dims in the returned state) or adopt explicitly via `open {url, relay:true}` per site. Symptom to know: `tab.tab.screenshot` is a code typo (nested property), not a target-miss — a real target-miss fails earlier at `goto`/`evaluate`.
