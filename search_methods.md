...
- **D53 addendum — leadprysm RSC extraction vs rendered authority (cross-checked 2026-08-25, post-close):** rendered-page check on the live august feed (headless Chrome, leadprysm.com/funded/august-2026): page header states **"91 companies tracked this month"**, while `parse_feed` on the same feed's payload returns **60 cards** — extraction is incomplete versus the rendered total (whether the RSC payload itself carries only a page-1 subset, or `parse_feed` omits rows, was NOT inspected; the payload's pagination/row structure remains unverified). Either way, a pool built from the extraction alone silently misses ~31 companies of the month. First-screen rendered cards (Rezolv, Smack Technologies, Etched, …) match parse_feed's leading rows in order — extraction↔rendered titles agree at the front, but the count differs. Also observed: the rendered list shows "Smack Technologies" and "Smack" as separate adjacent cards for the same $61M Aug 18 round — identity **verified** from the app's own data: both complete card subtrees (exhaustive per-row parse) carry the IDENTICAL logo-favicon URL, resolving to the canonical company domain `smacktechnologies.com` — the shared reference both rows resolve to; no href/url/link/id keys exist in either row, only that logo `src`. The rows differ purely in presentation (one carries location + Series B, the other neither). **Tooling note:** LeadPrysm card domains are NOT stored in a domain field — `parse_feed`["domain"] is null on these rows because the domain lives inside the logo favicon `src` URL (`…/api/public/logo?u=…favicons?domain=<company>.com…`); parse that URL for domain evidence. Card titles are candidate labels, not identity — verify via shared domain/link references, not name/amount/date matching alone. **Rule for D54+:** treat any RSC/regex card extraction as non-authoritative coverage until the payload's pagination/row structure is inspected and matched against the rendered total; prefer the rendered header total as the month authority; keep TC-wire per-story cross-checks for domain/date attribution (RSC rows still omit domains, e.g. Rezolv/Smack).
