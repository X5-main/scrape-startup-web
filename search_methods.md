...
- **D53 addendum — leadprysm RSC payload vs rendered authority (cross-checked 2026-08-25, post-close):** rendered-page check on the live august feed (headless Chrome, leadprysm.com/funded/august-2026): page header states **"91 companies tracked this month"**, while `parse_feed` on the same feed's RSC payload returns **60 cards** — the payload is a partial fragment, so a pool built from the payload alone silently misses ~31 companies of the month. First-screen rendered cards (Rezolv, Smack Technologies, Etched, …) match parse_feed's leading rows in order — payload↔rendered titles agree at the front, but the count differs. Also observed: the rendered list carries a duplicate ("Smack Technologies" and "Smack" as separate cards for the same $61M Aug 18 round) — card titles are candidate labels, not identity. **Rule for D54+:** treat any RSC/regex card extraction as a fragment until proven against the rendered page; prefer the rendered header total as the month authority; keep TC-wire per-story cross-checks for domain/date attribution (RSC rows still omit domains, e.g. Rezolv/Smack).
