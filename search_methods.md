...
2. **Candidate gate on F26:** exhaust F26's 18 companies first (they were never swept in D15/D16 which only did W26/S26). After www-normalized exclusion of the 18 existing mirror domains + content-signal-robots skip class, **herdr.dev** (YC F26, "Command line for AI agents", agentic dev-tool infrastructure) is the pick — clean robots (only `/fonts/` disallowed), funded gate clear.
