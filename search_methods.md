...
- **Lessons for D45:** (1) `Target.createTarget newWindow:true` does not guarantee a new window — control which tab the window displays via `Target.activateTarget` before WindowServer captures (no OS raise needed; in-window only). (2) `getComputedStyle().opacity === '0'` on an SSR element = animation-state delta, NOT missing content — check it before flagging a mirror incomplete. (3) cua-driver `get_window_state` carries the screenshot inline (`screenshot_png_b64`); decode + md5-fix as the evidence pair (rep 475,512 B / live 356,759 B); the daemon writes no new shots-dir PNG for window captures. (4) For animation-heavy heroes, DOM-position + heading diff is the decisive content-parity test; full-page pixel diffs mislead when live elements sit at opacity 0.
