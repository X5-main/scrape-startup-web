...
-
- **D53 addendum — leadprysm RSC parser validated 2026-08-25 02:2x UTC (post-close):** `parse_feed` re-run on a fresh `https://leadprysm.com/funded/august-2026` fetch (216099B, same byte class as D53's capture — no layout change since) returns **60 cards with well-formed fields** (title/catloc/amount/day-level date; `domain` present on some rows, `None` on most — e.g. Rezolv, Smack Technologies → domain still NOT embedded in the RSC card; resolve via web_search as D53 did). Spot-validation against the TC funding wire: Smack Technologies $61M Aug 18 matches the $61M Series B story — card↔story cross-check passes. **Rule recorded for D54+:** any schema/regex-derived pool MUST be validated against the rendered page (card count + several known titles) before ranking; the RSC layout is a moving target and a layout change made the older schema return zero silently in an earlier round — an unvalidated split can misassociate domain/date fields.
