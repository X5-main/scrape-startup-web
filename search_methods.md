...
- **d55_stability** (source `https://stability.ai/` live fetch, the D55 backup candidate, NOT mirrored): **6.9/10 → JUDGE** | html 525381B, text 6351 chars, h1 0 / h2 13 / h3 1, nav 4 footer 1 imgs 58, canvas 4, logo-wall 5, fonts: 14 @font-face, display `archivo`/`figtree`, dark False, animation three.js, **boilerplate: ['squarespace']**. **Material qualification:** the −0.35 template penalty is already inside the 6.9; without it the pre-filter would be ~7.25. Squarespace boilerplate was a real factor in the D55 vision 5 (template-thin reading of a rich-markup page) — if stability.ai ever wins a future round on visuals, this signal record must cap the claim (template-credit in score, not bespoke design). Same rule for any candidate flagged `w-webflow|wix|squarespace|elementor|...` by the BOILERPLATE regex.
- **Rule (D56+):** run capture_signal.py on every candidate pre-screening AND on the elected mirror's index.html post-mirror; commit the JSONs with the round; a non-empty `boilerplate` field goes verbatim into the round README row as a qualification.
