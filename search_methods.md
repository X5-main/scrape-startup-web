...
- **Lesson for D50:** before declaring a docs-engined mirror complete, (1) capture the target docs route LIVE and diff its resource list (`.md` + assets) against the mirror — the runtime graph is the authority; (2) check the module-graph closure for dynamic `import()` chunks reachable only through a route that the static crawl never renders; (3) after introducing redirect support in serve_replica.py, verify with a FRESH probe process — the old process's log can linger under the same hub name.
