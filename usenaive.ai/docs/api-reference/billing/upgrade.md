...
````yaml /api-reference/openapi.json post /v1/billing/upgrade
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
            ...
            schema:
              ...
              properties:
                plan:
                  type: string
                  description: >-
                    The plan to move to. Only offered plans are listed, and
                    'pro' is always a valid value to send here. If you already
                    have a live subscription — including one on a retired plan —
                    this endpoint ignores the value and returns a Stripe billing
                    portal URL, where the plan change is actually made; a
                    retired plan id you are currently on is accepted for that
                    path too, but you never need to send it.
                  enum:
                    - pro
      responses:
        '200':
