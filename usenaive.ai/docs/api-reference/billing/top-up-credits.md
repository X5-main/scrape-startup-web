...
> Buy a credit pack. Purchased credit is what unlocks LLM routing: any completed top-up clears `block_reason: "llm_routing_requires_payment"` for the company permanently, in a way that free grants and verified email do not.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/billing/topup
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/billing/topup:
    post:
      ...
      description: >-
        Buy a credit pack. Purchased credit is what unlocks LLM routing: any
        completed top-up clears `block_reason: "llm_routing_requires_payment"`
        for the company permanently, in a way that free grants and verified
        email do not.
      requestBody:
        required: true
        content:
