...
> Start a subscription. A paid subscription is the other way to unlock LLM routing: it clears `block_reason: "llm_routing_requires_payment"`, which free signup credits do not.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/billing/subscribe
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/billing/subscribe:
    post:
      ...
      summary: Subscribe
      description: >-
        Start a subscription. A paid subscription is the other way to unlock LLM
        routing: it clears `block_reason: "llm_routing_requires_payment"`, which
        free signup credits do not.
      requestBody:
        required: true
        content:
          application/json:
