...
> Sensitive card credentials (active cards only).



## OpenAPI

````yaml /api-reference/openapi.json get /v1/cards/{id}/details
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/cards/{id}/details:
    get:
      ...
      description: Sensitive card credentials (active cards only).
      parameters:
        - name: id
          in: path
          required: true
