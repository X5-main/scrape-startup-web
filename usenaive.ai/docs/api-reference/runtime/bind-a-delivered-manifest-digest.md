...
# Bind a delivered manifest digest

> REAL on the durable runtime, 501 on the legacy one. This description used to read '501 in this build … there is no applied digest to compare `expected_digest` against', and both halves of that were about the wrong runtime: a durable tenant's runtime keeps `company_apply` and raises exactly that 409. `PUT …/manifest` (which `naive up` drives) STORES a compiled definition and answers with its digest; this binds that exact digest, and until one is bound the team's dispatcher ticks nothing and every intake answers 409. `expected_digest` is REQUIRED — an apply with no digest would bind whatever the registry currently holds, which is the review bypass the two steps exist to prevent. Read the digest from `GET …/plan` (`manifest_digest`). A digest that has been overtaken is refused with the runtime's own 409 carrying BOTH digests, surfaced as `provider_error` with `details.runtime_status: 409`.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/teams/{team}/tenants/{tenantUserId}/apply
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/apply:
    post:
      tags:
        - Runtime
      summary: Bind a delivered manifest digest
      description: >-
        REAL on the durable runtime, 501 on the legacy one. This description
        used to read '501 in this build … there is no applied digest to compare
        `expected_digest` against', and both halves of that were about the wrong
        runtime: a durable tenant's runtime keeps `company_apply` and raises
        exactly that 409. `PUT …/manifest` (which `naive up` drives) STORES a
        compiled definition and answers with its digest; this binds that exact
        digest, and until one is bound the team's dispatcher ticks nothing and
        every intake answers 409. `expected_digest` is REQUIRED — an apply with
        no digest would bind whatever the registry currently holds, which is the
        review bypass the two steps exist to prevent. Read the digest from `GET
        …/plan` (`manifest_digest`). A digest that has been overtaken is refused
        with the runtime's own 409 carrying BOTH digests, surfaced as
        `provider_error` with `details.runtime_status: 409`.
      parameters:
        - name: team
          in: path
          required: true
          schema:
            type: string
          description: Team slug. Lowercase letters, digits and hyphens, 1..63 characters.
        - name: tenantUserId
          in: path
          required: true
          schema:
            type: string
          description: >-
            The tenant_user this team is bound to. `default` and `me` resolve to
            ...
            schema:
              ...
              properties:
                expected_digest:
                  type: string
                  description: >-
                    The compiled manifest digest the caller reviewed, as `GET
                    …/plan` reports it. Lowercase sha256 hex; the runtime
                    validates the shape.
      responses:
        '200':
          description: >-
            Bound. `bound_now` and `already_bound` keep a bind that CHANGED the
            binding apart from one that confirmed it — both are successes.
          content:
            application/json:
              schema:
                type: object
                properties:
                  ...
                  bound_now:
                    type: boolean
                    description: This call recorded a new binding.
                  already_bound:
                    type: boolean
                    description: That digest was already bound; nothing changed.
                  changes:
                    type: array
                    items:
                      type: object
