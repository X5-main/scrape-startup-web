...
> REAL, and the only write on this surface that is not refused. It is a second ADDRESS for one write path — the same `executeApproval`/`denyApproval` the existing approvals routes call — so the human-resolver rule, the no-self-approval rule and the frozen-payload replay all hold identically. `because` is required on a deny.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/teams/{team}/tenants/{tenantUserId}/approvals/{id}/decide
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/approvals/{id}/decide:
    post:
      tags:
        - Runtime
      summary: Approve or deny
      description: >-
        REAL, and the only write on this surface that is not refused. It is a
        second ADDRESS for one write path — the same
        `executeApproval`/`denyApproval` the existing approvals routes call — so
        the human-resolver rule, the no-self-approval rule and the
        frozen-payload replay all hold identically. `because` is required on a
        deny.
      parameters:
        - name: team
          in: path
