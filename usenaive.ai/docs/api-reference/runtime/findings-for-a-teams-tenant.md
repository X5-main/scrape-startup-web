...
# Findings for a team's tenant

> Computes the findings the mirror can actually support — stranded (a claimed card whose lease lapsed) and unverified — and names the four it cannot, with the reason for each.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/diagnostics
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/diagnostics:
    get:
      tags:
        - Runtime
      summary: Findings for a team's tenant
      description: >-
        Computes the findings the mirror can actually support — stranded (a
        claimed card whose lease lapsed) and unverified — and names the four it
