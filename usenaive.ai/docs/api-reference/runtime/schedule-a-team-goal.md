...
# Schedule a team goal



## OpenAPI

````yaml /api-reference/openapi.json post /v1/teams/{team}/tenants/{tenantUserId}/schedule
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
            ...
            schema:
              ...
              properties:
                cron:
                  type: string
                  description: >-
                    A five-field cron expression, evaluated in UTC. `timezone`
                    is NOT accepted and is refused with `invalid_input`: the
                    durable runtime's recurrence spec is a cron expression or an
                    interval and carries no zone, so a zone sent here would be
                    accepted and silently ignored while the job fired at the UTC
                    reading of the same expression.
                goal:
                  type: string
                  description: What to submit on each firing.
                brief:
                  type: string
                  description: >-
                    The prose each firing is worked from. Forwarded to the
                    runtime as the seeded card's `brief`; a runtime that cannot
                    consume it names it back in `unsupported_fields`.
      responses:
        '401':
