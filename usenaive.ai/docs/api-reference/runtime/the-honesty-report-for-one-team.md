...
# The honesty report for one team

> Prints the four mechanisms that claim a property the platform does not have — the idempotency store, read-side revocation, the connections mode and the deprecation headers — all four or none. A report that prints one and hides three is selectively honest. For a DURABLE tenant it also asks that tenant's own runtime for its manifest binding, so `manifest_digest`, `applied_digest` and `fence` (`"bound"` | `"unbound"`) are the runtime's own values rather than nulls, and `manifest_stale` names both digests, the effect, and the `POST …/apply` call that repairs it. Staleness is DETECTED AND REPORTED, NEVER REPAIRED: a control plane that re-binds on drift deletes the human review the digest exists for. `manifest_digest` is COMPANY-scoped (`digest_scope`) — one registry row per company, so every durable team of a company reads and binds the same digest. This is a REPORT and degrades rather than failing: a runtime that does not answer yields nulls with the reason in `unavailable_because.digests`, never a fabricated digest and never a 5xx.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/plan
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/plan:
    get:
      tags:
        - Runtime
      summary: The honesty report for one team
      description: >-
        Prints the four mechanisms that claim a property the platform does not
        have — the idempotency store, read-side revocation, the connections mode
        and the deprecation headers — all four or none. A report that prints one
        and hides three is selectively honest. For a DURABLE tenant it also asks
        that tenant's own runtime for its manifest binding, so
        `manifest_digest`, `applied_digest` and `fence` (`"bound"` |
        `"unbound"`) are the runtime's own values rather than nulls, and
        `manifest_stale` names both digests, the effect, and the `POST …/apply`
        call that repairs it. Staleness is DETECTED AND REPORTED, NEVER
        REPAIRED: a control plane that re-binds on drift deletes the human
        review the digest exists for. `manifest_digest` is COMPANY-scoped
        (`digest_scope`) — one registry row per company, so every durable team
        of a company reads and binds the same digest. This is a REPORT and
        degrades rather than failing: a runtime that does not answer yields
        nulls with the reason in `unavailable_because.digests`, never a
        fabricated digest and never a 5xx.
      parameters:
        - name: team
          in: path
          required: true
      ...
      responses:
        '200':
          description: The honesty report.
          content:
            application/json:
              schema:
                type: object
