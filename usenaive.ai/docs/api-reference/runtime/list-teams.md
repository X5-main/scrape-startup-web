...
> The only company-scoped operation on this surface, and the only one that takes no `--tenant`. Serves the MANIFEST half of each declared team — name, project, runtime, lead, roles, the tenant ids its other operations are addressed by, and whether it is placed. The per-(team, tenant) aggregates (board counts, spend, pending approvals) are not here and cannot be: this operation resolves no tenant. Read them from `GET /v1/teams/{team}/tenants/{tenantUserId}`.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/teams:
    get:
      ...
      summary: List teams
      description: >-
        The only company-scoped operation on this surface, and the only one that
        takes no `--tenant`. Serves the MANIFEST half of each declared team —
        name, project, runtime, lead, roles, the tenant ids its other operations
        are addressed by, and whether it is placed. The per-(team, tenant)
        aggregates (board counts, spend, pending approvals) are not here and
        cannot be: this operation resolves no tenant. Read them from `GET
        ...
        - name: runtime
          ...
          schema:
            ...
            enum:
              - durable
              - hermes
          description: Only teams whose members are on this runtime.
        - name: project
          in: query
          required: false
          schema:
            type: string
          description: Only teams declared under this project.
      responses:
        '200':
          description: >-
            The declared teams, by name. An empty list is an honest 200 and not
            a 404: a company that has applied no config has no declared teams,
            which is a fact about the company rather than a missing address.
          content:
            application/json:
              schema:
                type: object
                    ...
                    items:
                      ...
                      properties:
                        team:
                          type: string
                          description: >-
                            The declared name — the `{team}` path segment every
                        ...
                        runtime:
                          ...
                          nullable: true
                          description: >-
                            `mixed` is a REAL answer for a team with members on
                            both runtimes. `null` means no live member records
                            `config_runtime` — rows predating that key — and is
                            served with a sibling `runtime_unavailable_because`
                            rather than defaulting to `hermes`.
                        tenant_user_ids:
                          type: array
                          items:
                            type: string
                          description: >-
                            The `--tenant` every other verb on this surface
                            requires. A LIST: a mixed team addresses more than
                            one subject, and collapsing it would hand a caller
                            an id that reaches half its roster.
                        lead:
                          type: string
                          nullable: true
                          description: >-
                            The lead's role, from `metadata.config_root`. Null
                            when no live row claims it.
                        roles:
                          type: array
                          items:
                            type: string
                        unplaced:
                          type: boolean
                          description: >-
                            At least one live member is `provision_status =
                            'unplaced'` — read from the same column the
                            deployment status reads, so this list and that
                            screen cannot disagree about whether a declared team
                            is reachable.
                        declared_at:
                          type: string
                          format: date-time
                        fence:
                          ...
                          nullable: true
                          description: >-
                            Null in this build — the apply fence needs an
                            applied digest to compare against and no manifest
                            store exists. See `unavailable_because`.
                        digests:
                          type: object
                          nullable: true
                          properties:
                            ...
                            snapshot:
                              type: string
                          description: Null in this build. See `unavailable_because`.
                        brain_partition:
                          type: string
                          nullable: true
                          description: >-
                            Null in this build — a manifest field. See
                            `unavailable_because`.
                        runtime_unavailable_because:
                          type: string
                          description: Present only when `runtime` is null, naming why.
                  next_cursor:
                    type: string
                    nullable: true
                    description: >-
                      Present and null: this list is bounded by the declared
                      config rather than by traffic, so it is served whole.
                      Published so a client that pages every list on this
                      surface reads one shape everywhere.
                  unavailable_because:
                    type: object
                    description: >-
                      Why `fence`, `digests` and `brain_partition` are null, in
                      the API's own words. Null with a reason, never omitted —
                      an omitted `fence` reads as "this team has no fence".
        '401':
