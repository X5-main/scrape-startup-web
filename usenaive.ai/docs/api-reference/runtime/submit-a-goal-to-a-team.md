...
````yaml /api-reference/openapi.json post /v1/teams/{team}/tenants/{tenantUserId}/submit
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/submit:
    post:
      tags:
        - Runtime
      summary: Submit a goal to a team
      description: >-
        501 in this build. Three dependencies are absent and each alone would
        make the response dishonest: the durable dispatcher, manifest-digest
        storage, and an idempotency store that can honour `idempotency_key` (the
        current one is an in-process Map with no body fingerprint and no 2xx
        filter).
      parameters:
        - name: team
          in: path
