...
# Team session channels

> 501 in this build. `company_channel_messages` is company-scoped and carries no team or tenant column, so re-serving it here would answer a different question under this address.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/sessions
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/sessions:
    get:
      tags:
        - Runtime
      summary: Team session channels
      description: >-
        501 in this build. `company_channel_messages` is company-scoped and
        carries no team or tenant column, so re-serving it here would answer a
        different question under this address.
      parameters:
        - name: team
          in: path
          required: true
