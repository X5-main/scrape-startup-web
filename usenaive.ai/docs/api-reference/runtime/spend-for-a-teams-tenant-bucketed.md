...
# Spend for a team's tenant, bucketed

> REAL rows from `tenant_spend_events` over the tenant's OWN budget window — not a calendar month, because the cap it is compared against resets on that window. Every bucket carries `enforced`, and it is false here: nothing in this build refuses a call on the basis of these numbers.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/cost
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/cost:
    get:
      tags:
        - Runtime
      summary: Spend for a team's tenant, bucketed
      description: >-
        REAL rows from `tenant_spend_events` over the tenant's OWN budget window
        — not a calendar month, because the cap it is compared against resets on
        that window. Every bucket carries `enforced`, and it is false here:
        nothing in this build refuses a call on the basis of these numbers.
      parameters:
        - name: team
          in: path
          required: true
        ...
        - name: by
          ...
          schema:
            type: string
          description: Only `action` is computable in this build.
      responses:
        '200':
          description: Cost buckets.
          content:
            application/json:
              schema:
                type: object
