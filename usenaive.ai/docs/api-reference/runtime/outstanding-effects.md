...
> 501 rather than an empty list: `{items: []}` would say this tenant has no outstanding effects — a claim about the tenant — rather than that there is no effect ledger, which is a claim about the build.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/effects
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/effects:
    get:
      tags:
        - Runtime
      summary: Outstanding effects
      description: >-
        501 rather than an empty list: `{items: []}` would say this tenant has
        no outstanding effects — a claim about the tenant — rather than that
        there is no effect ledger, which is a claim about the build.
      parameters:
        - name: team
          in: path
          required: true
