...
> 501 in this build. `POST /v1/tasks/{id}/unblock` unblocks a legacy card and keeps answering; this address needs the durable board and a decision ledger.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/teams/{team}/tenants/{tenantUserId}/board/{card}/unblock
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/board/{card}/unblock:
    post:
      tags:
        - Runtime
      summary: Unblock a card
      description: >-
        501 in this build. `POST /v1/tasks/{id}/unblock` unblocks a legacy card
        and keeps answering; this address needs the durable board and a decision
        ledger.
      parameters:
        - name: team
          in: path
