...
````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/board/{card}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/board/{card}:
    get:
      tags:
        - Runtime
      summary: One card, its runs and where its events live
      parameters:
        - name: team
          in: path
          required: true
        ...
        - name: card
          ...
          schema:
            type: string
          description: The board id (`tasks_mirror.hermes_task_id`).
      responses:
        '200':
              ...
              schema:
                ...
                properties:
                  ...
                  row_id:
                    type: string
                    description: >-
                      The naive UUID for the same card. Reported beside the
                      board id, never instead of it.
                  title:
                    type: string
                  status:
                    type: string
                    enum:
                      ...
                      - blocked
                    description: >-
                      The six board columns. Computed from `tasks_mirror.status`
                      AND `verification_status` until R-6's CHECK makes their
                  ...
                  role:
                    ...
                    description: >-
                      ...
                      `tasks_mirror.assignee`, which keeps its column name.
                  attempts:
                    type: integer
                  lease:
                    type: object
                    ...
                    properties:
                      role:
                        type: string
                        description: >-
                          The role holding the lease — the same value as the
                          card's `role`.
                      expires_at:
                        type: string
                        format: date-time
                  ...
                  rubric:
                    ...
                    nullable: true
                    description: >-
                      Always null in this build: there is no per-card check
                      list. Null, not `{passed:0,total:0}` — which would read as
                      `no checks passed`.
                  ...
                  unverified_because:
                    ...
                    nullable: true
                    description: >-
                      REQUIRED whenever `status` is `unverified`. A terminal
                      state a user cannot explain is a broken product.
        '401':
