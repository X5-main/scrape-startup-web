...
> The header the board and the CLI print above everything: runtime provider, the six board counts, spend for the tenant's own budget window, and — reported as null WITH a reason rather than as a plausible zero — the digests, the fence and the stop state, none of which have storage in this build.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/teams/{team}/tenants/{tenantUserId}:
    get:
      tags:
        - Runtime
      summary: Get a team at a tenant
      description: >-
        The header the board and the CLI print above everything: runtime
        provider, the six board counts, spend for the tenant's own budget
        window, and — reported as null WITH a reason rather than as a plausible
        zero — the digests, the fence and the stop state, none of which have
      ...
      responses:
        '200':
          description: The team/tenant header.
          content:
            application/json:
              schema:
                type: object
                properties:
                  ...
                  spend:
                    ...
                    properties:
                      ...
                      enforced:
                        type: boolean
                        description: FALSE means declared but not enforced.
                  period:
                    type: object
                    properties:
                      name:
