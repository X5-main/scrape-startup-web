...
# What recurs for this team

> The GET side of `POST …/schedule`. The POST returns the job id ONCE and `DELETE …/schedule/{id}` is addressed by that id, so without this read a registered job that keeps spending on a timer has no handle an operator can recover. Served for a durable tenant; `501 not_configured` for a hermes one, whose recurring work is company-scoped `cron_jobs` with no team column.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/schedule
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/schedule:
    get:
      tags:
        - Runtime
      summary: What recurs for this team
      description: >-
        The GET side of `POST …/schedule`. The POST returns the job id ONCE and
        `DELETE …/schedule/{id}` is addressed by that id, so without this read a
        registered job that keeps spending on a timer has no handle an operator
        can recover. Served for a durable tenant; `501 not_configured` for a
        hermes one, whose recurring work is company-scoped `cron_jobs` with no
        team column.
      parameters:
        - name: team
          in: path
      ...
      responses:
        '200':
          description: >-
            Every job registered for this (team, tenant). Not paged — the
            runtime answers with all of them and a team may hold at most a fixed
            few.
          content:
            application/json:
              schema:
                ...
                properties:
                    ...
                    items:
                      ...
                      properties:
                        id:
                          type: string
                          description: >-
                            The job id. This is what `DELETE …/schedule/{id}`
                            takes.
                        schedule_id:
                          type: string
                          nullable: true
                          description: >-
                            The runtime's own alarm-row id, reported beside the
                            job id and never instead of it.
                        cron:
                          type: string
                          nullable: true
                          description: >-
                            The cron expression, evaluated in UTC. Null when the
                            job recurs on an interval.
                        every_seconds:
                          type: integer
                          nullable: true
                          description: >-
                            The interval in seconds. Null when the job recurs on
                            a cron expression. Exactly one of `cron` and
                            `every_seconds` is non-null.
                        next_run_at:
                          type: string
                          format: date-time
                          nullable: true
                          description: When this job next fires.
                        model:
                          type: string
                          nullable: true
                          description: >-
                            The model this job was PINNED to at registration.
                            Null means it uses whatever the roles default to at
                            firing time; a value means the job keeps spending on
                            that model regardless of what the team has been
                            repointed at since.
                        goal:
                          type: string
                          description: What each firing submits.
                        task_template_id:
                          type: string
                          nullable: true
                          description: The id of the card template each firing seeds.
                        registered_by:
                          type: string
                          nullable: true
                          description: >-
                            The subject that registered this standing permission
                            to spend.
                        registered_at:
                          type: string
                          format: date-time
                  ...
                  next_cursor:
                    type: string
                    nullable: true
                    description: Always null — this route takes no cursor.
                  next_cursor_unavailable_because:
                    type: string
        '401':
