...
> Who is on the team, from `agents`. `can`/`approve`/`view` are null rather than `[]` because an empty ability list is a MEANINGFUL value and reporting `unknown` as `none` would understate authority in the one direction an operator must never be misled about.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/teams/{team}/tenants/{tenantUserId}/roster
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/teams/{team}/tenants/{tenantUserId}/roster:
    get:
      tags:
        - Runtime
      summary: The team roster
      description: >-
        Who is on the team, from `agents`. `can`/`approve`/`view` are null
        rather than `[]` because an empty ability list is a MEANINGFUL value and
        reporting `unknown` as `none` would understate authority in the one
        direction an operator must never be misled about.
      parameters:
        - name: team
          in: path
          required: true
