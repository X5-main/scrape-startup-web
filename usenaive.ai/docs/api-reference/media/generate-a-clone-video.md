...
# Generate a clone video

> Submit an async clone_generate job (reference image + cloned voice + script). Private beta.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/clone/generate
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/clone/generate:
    post:
      tags:
        - Media
      summary: Generate a clone video
      description: >-
        Submit an async clone_generate job (reference image + cloned voice +
        script). Private beta.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                voice_id:
                  type: string
                  description: Existing cloned voice profile id.
                image_url:
                  type: string
                  description: Public https URL of the reference image.
                text:
                  type: string
                  description: Script the cloned voice speaks.
                scene:
                  type: string
                  description: Optional scene to relight the subject into.
                quality:
                  type: string
                  enum:
                    - max
                    - fast
                  description: Animation tier.
                i_affirm:
                  type: boolean
                  description: Must be true; affirms likeness consent.
      responses:
        '202':
