...
# Business data (task post)

> Submit a business-data task. `platform`: google, trustpilot, tripadvisor. `endpoint` varies per platform (e.g. reviews, search, my-business-updates).



## OpenAPI

````yaml /api-reference/openapi.json post /v1/business/{platform}/{endpoint}/task
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/business/{platform}/{endpoint}/task:
    post:
      tags:
        - Business Data
      summary: Business data (task post)
      description: >-
        Submit a business-data task. `platform`: google, trustpilot,
        tripadvisor. `endpoint` varies per platform (e.g. reviews, search,
        my-business-updates).
      parameters:
        - name: platform
          in: path
