...
> Reviews for place ids from POST /v1/business/places/search.

REVIEWER IDENTITY IS SWITCHED OFF at the provider, whose default is to attach it. This returns the review — rating, text, date, the owner's reply — because a reviewer's cross-business review history is a behavioural profile of a private individual and nothing in this primitive's terms covers it.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/business/places/reviews
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/business/places/reviews:
    post:
      tags:
        - Business Data
      summary: Read reviews for known places (Google Maps)
      description: >-
        Reviews for place ids from POST /v1/business/places/search.


        REVIEWER IDENTITY IS SWITCHED OFF at the provider, whose default is to
        attach it. This returns the review — rating, text, date, the owner's
        reply — because a reviewer's cross-business review history is a
        behavioural profile of a private individual and nothing in this
        primitive's terms covers it.
      parameters:
            ...
            schema:
              ...
              properties:
                place_ids:
                  ...
                  items:
                    type: string
                  description: Max 25 per request.
                max_reviews_per_place:
                  type: number
                  default: 20
                  description: >-
                    1-100, and it MULTIPLIES: 25 places x 100 reviews is 2,500
                    billed rows from one call.
                sort:
                  type: string
                  enum:
                    - newest
      ...
      responses:
        ...
        '501':
          description: Not configured — the Maps provider has no credential.
components:
  schemas:
    BusinessReview:
      type: object
      description: >-
        A review WITHOUT reviewer identity: no name, profile URL, photo or
        cross-business review history is collected. See the Reviews & Listings
        terms.
      properties:
        id:
          type: string
        ...
        text:
          type: string
          description: >-
            Empty for a rating left without words — still a real review, so the
            row is kept.
        published_at:
          type: string
          format: date-time
        ...
        owner_response:
          type: string
          description: The business owner's reply, where there is one.
    Error:
      type: object
      properties:
        error:
