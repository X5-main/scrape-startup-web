...
> Create an LLC formation and a $349 Stripe checkout. Approval-gated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/formation
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/formation:
    post:
      ...
      description: Create an LLC formation and a $349 Stripe checkout. Approval-gated.
      requestBody:
        required: true
        content:
          application/json:
      ...
      responses:
        '201':
          description: Created (awaiting payment).
          content:
            application/json:
              schema:
                type: object
...
components:
  schemas:
    PendingApproval:
      type: object
      description: >-
        Returned with HTTP 202 when an agent action is gated behind a human
        approval.
      properties:
        status:
          type: string
