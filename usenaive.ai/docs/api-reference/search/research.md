...
> AI research synthesis. `quick` is synchronous and costs 0.5 credits, charged on success; `thorough` (1 credits) and `exhaustive` (1.6 credits) return an async job and are charged on completion. An unknown depth is rejected rather than billed as `quick`, and a job that returns no sources and no synthesis fails without a charge. Returns 402 when the balance is below the full price of the requested depth.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/search/research
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/search/research:
    post:
      ...
      summary: Research
      description: >-
        AI research synthesis. `quick` is synchronous and costs 0.5 credits,
        charged on success; `thorough` (1 credits) and `exhaustive` (1.6
        credits) return an async job and are charged on completion. An unknown
        depth is rejected rather than billed as `quick`, and a job that returns
        no sources and no synthesis fails without a charge. Returns 402 when the
        balance is below the full price of the requested depth.
      requestBody:
        required: true
        content:
          application/json:
      ...
      responses:
        '200':
          description: Synchronous result (quick).
          content:
            application/json:
              schema:
                type: object
        ...
        '202':
          description: Async job (thorough/exhaustive).
          content:
            application/json:
              schema:
                type: object
