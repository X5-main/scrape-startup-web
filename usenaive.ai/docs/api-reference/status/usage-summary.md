...
> Credit transaction history. Charges priced from real provider spend (the People, Company Data, Social Data and Reviews & Listings primitives) also carry `provider_usd` — what the vendor charged for that request — `provider_usd_basis` (`actual` when measured off the run, `projected` when estimated) and `margin_multiplier`, so credits = provider_usd × margin_multiplier can be verified. Flat-rate meters omit them.
...
paths:
  /v1/usage:
    get:
      ...
      description: >-
        Credit transaction history. Charges priced from real provider spend (the
        People, Company Data, Social Data and Reviews & Listings primitives)
        also carry `provider_usd` — what the vendor charged for that request —
        `provider_usd_basis` (`actual` when measured off the run, `projected`
        when estimated) and `margin_multiplier`, so credits = provider_usd ×
        margin_multiplier can be verified. Flat-rate meters omit them.
      parameters:
        - name: days
          in: query
          schema:
