...
> Live DataForSEO Labs query. `engine` is one of: google, bing, amazon. See https://docs.dataforseo.com for the per-engine endpoint list (e.g. ranked-keywords, keyword-ideas, competitors-domain, bulk-keyword-difficulty). Keyword endpoints (e.g. keyword-overview) take `keywords` (array of strings) and require a location and language: `location_code` (default `2840`, United States) and `language_code` (default `en`) are applied automatically when omitted, and can be overridden.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/seo/labs/{engine}/{endpoint}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/seo/labs/{engine}/{endpoint}:
    post:
      tags:
        - SEO
      summary: Labs (live)
      description: >-
        Live DataForSEO Labs query. `engine` is one of: google, bing, amazon.
        See https://docs.dataforseo.com for the per-engine endpoint list (e.g.
        ranked-keywords, keyword-ideas, competitors-domain,
        bulk-keyword-difficulty). Keyword endpoints (e.g. keyword-overview) take
        `keywords` (array of strings) and require a location and language:
        `location_code` (default `2840`, United States) and `language_code`
        (default `en`) are applied automatically when omitted, and can be
        overridden.
      parameters:
        - name: engine
          in: path
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/DataForSEOTaskParams'
      responses:
        '200':
          description: OK.
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/DataForSEOResponse'
        '402':
          $ref: '#/components/responses/PaymentRequired'
components:
  schemas:
    DataForSEOTaskParams:
      type: object
      description: >-
        Opaque DataForSEO task parameters, forwarded upstream as-is. See
        https://docs.dataforseo.com for per-endpoint fields.
      additionalProperties: true
    DataForSEOResponse:
      type: object
      description: DataForSEO passthrough envelope.
      properties:
        tasks:
          type: array
          items:
        ...
        credits_used:
          type: number
          description: Present on billed POST/live endpoints.
        credits_remaining:
          type: number
          description: Present on billed POST/live endpoints.
    Error:
