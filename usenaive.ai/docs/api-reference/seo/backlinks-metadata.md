...
> When `endpoint` is `index` or `available-filters`, returns Backlinks API metadata instead of running a query.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/seo/backlinks/{endpoint}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/seo/backlinks/{endpoint}:
    get:
      tags:
        - SEO
      summary: Backlinks metadata
      description: >-
        When `endpoint` is `index` or `available-filters`, returns Backlinks API
        metadata instead of running a query.
      parameters:
        - name: endpoint
          in: path
          required: true
