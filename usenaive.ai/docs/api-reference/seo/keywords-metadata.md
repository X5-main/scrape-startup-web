...
> When the second path segment is a metadata type — `locations`, `languages`, `categories`, or `adwords-status` (Google only) — returns Keywords Data metadata for the engine instead of running a query.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/seo/keywords/{engine}/{endpoint}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/seo/keywords/{engine}/{endpoint}:
    get:
      ...
      description: >-
        When the second path segment is a metadata type — `locations`,
        `languages`, `categories`, or `adwords-status` (Google only) — returns
        Keywords Data metadata for the engine instead of running a query.
      parameters:
        - name: engine
          in: path
          required: true
