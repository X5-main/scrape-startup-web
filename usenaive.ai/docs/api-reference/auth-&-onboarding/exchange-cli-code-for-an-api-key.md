...
# Exchange CLI code for an API key

> Redeems a one-time `cli`-mode exchange code for a primary API key.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/cli/exchange
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/cli/exchange:
    post:
      tags:
        - Auth & Onboarding
      summary: Exchange CLI code for an API key
      description: Redeems a one-time `cli`-mode exchange code for a primary API key.
      requestBody:
        required: true
        content:
          application/json:
