...
> Redirect to the Google OAuth consent screen (browser flow).



## OpenAPI

````yaml /api-reference/openapi.json get /v1/auth/oauth/google
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/oauth/google:
    get:
      ...
      description: Redirect to the Google OAuth consent screen (browser flow).
      parameters:
        - name: redirect
          in: query
          schema:
            type: string
          description: Post-auth redirect base URL.
        - name: template
          in: query
          schema:
            type: string
          description: Template slug carried through the callback.
      responses:
        '302':
