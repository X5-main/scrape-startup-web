...
> Emails a one-time sign-in link that redirects back to the CLI loopback `redirect`.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/magic/start
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/magic/start:
    post:
      tags:
        - Auth & Onboarding
      summary: Start email magic-link sign-in
      description: >-
        Emails a one-time sign-in link that redirects back to the CLI loopback
        `redirect`.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                ...
                redirect:
                  type: string
                  description: Loopback URL, e.g. `http://127.0.0.1:PORT`.
                cli_state:
                  type: string
                  description: CSRF state echoed back on redirect.
      responses:
        '200':
          description: Link sent (or, in opt-in local dev, returned).
          content:
            application/json:
              schema:
                type: object
