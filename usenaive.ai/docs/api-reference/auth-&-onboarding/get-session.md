...
> Get the current session user/company context from the cookie.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/auth/session
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
  ...
  /v1/auth/session:
    get:
      tags:
        - Auth & Onboarding
      summary: Get session
      description: Get the current session user/company context from the cookie.
      responses:
        '200':
          description: OK.
          content:
