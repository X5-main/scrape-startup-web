...
# Retire predecessor agent keys

> Revokes every other tenant-scoped key for the calling agent, keeping the authenticating key.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/keys/retire-predecessors
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/auth/keys/retire-predecessors:
    post:
      tags:
        - Auth & Onboarding
      summary: Retire predecessor agent keys
      description: >-
        Revokes every other tenant-scoped key for the calling agent, keeping the
        authenticating key.
      responses:
        '200':
          description: Predecessors retired.
          content:
            application/json:
              schema:
                type: object
