...
> Email a password-reset link to an email+password account. Always returns the same body whether or not the account exists, so it is never an enumeration oracle.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/password/forgot
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/password/forgot:
    post:
      ...
      description: >-
        Email a password-reset link to an email+password account. Always returns
        the same body whether or not the account exists, so it is never an
        enumeration oracle.
      requestBody:
        required: true
        content:
            ...
            schema:
              ...
              properties:
                ...
                redirect:
                  type: string
                  description: >-
                    Absolute URL to land on after the reset. Must be an allowed
                    app origin or a loopback address.
      responses:
        '200':
              ...
              schema:
                ...
                properties:
                  ...
                  message:
                    type: string
        '400':
          $ref: 2b0dd563-d0ef-42e0-b97b-893a7d35d034
components:
  securitySchemes:
    bearerAuth:
      type: http
