...
> Revoke an API key owned by the authenticated agent.



## OpenAPI

````yaml /api-reference/openapi.json delete /v1/auth/keys/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/keys/{id}:
    delete:
      ...
      description: Revoke an API key owned by the authenticated agent.
      parameters:
        - name: id
          in: path
          required: true
