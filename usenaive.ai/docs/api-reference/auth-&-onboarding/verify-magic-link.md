...
> Claims the one-time token and redirects to the CLI loopback with a `cli` exchange code.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/auth/magic/verify
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/magic/verify:
    get:
      ...
      description: >-
        Claims the one-time token and redirects to the CLI loopback with a `cli`
        exchange code.
      parameters:
        - name: token
          in: query
          ...
          schema:
            type: string
          description: One-time magic-link token.
      responses:
        '302':
          description: Redirect to the CLI loopback with `code` and `cli_state`.
        '400':
          description: Link invalid or expired (HTML).
          content:
            text/html:
              schema:
                type: string
        '500':
          description: Sign-in failed (HTML).
          content:
            text/html:
              schema:
                type: string
