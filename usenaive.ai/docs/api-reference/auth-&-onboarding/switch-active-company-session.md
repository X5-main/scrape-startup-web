...
> Switch the active company in the current session.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/session/switch-company
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/auth/session/switch-company:
    post:
      tags:
        - Auth & Onboarding
      summary: Switch active company (session)
      description: Switch the active company in the current session.
      requestBody:
        required: true
        content:
          application/json:
