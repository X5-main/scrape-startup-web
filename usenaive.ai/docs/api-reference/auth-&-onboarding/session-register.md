...
> Session-based registration; provisions a company, default kit/user, CEO agent, and sends a verification email.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/session/register
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/auth/session/register:
    post:
      tags:
        - Auth & Onboarding
      summary: Session register
      description: >-
        Session-based registration; provisions a company, default kit/user, CEO
        agent, and sends a verification email.
      requestBody:
        required: true
        content:
          application/json:
      ...
      responses:
        '201':
          description: Registered (sets session cookie).
          content:
            application/json:
              schema:
                type: object
