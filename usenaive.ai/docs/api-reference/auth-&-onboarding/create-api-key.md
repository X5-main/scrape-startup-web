...
> Create a new API key for the authenticated agent, optionally scoped to an account kit and tenant user.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/keys
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/keys:
    post:
      ...
      description: >-
        Create a new API key for the authenticated agent, optionally scoped to
        an account kit and tenant user.
      requestBody:
        content:
          application/json:
            schema:
              ...
              properties:
                ...
                account_kit_id:
                  type: string
                  description: Seal the key to an account kit.
                active_tenant_user_id:
                  type: string
                  description: Default tenant user for the key.
                active_project_id:
                  type: string
                  description: >-
                    Pin the key to a project. Omitted leaves the key un-pinned:
                    it acts inside whichever project the request names, falling
                    back to the organization's default. A pinned key that is
                    handed a different project id answers 403
              ...
              schema:
                ...
                properties:
                  ...
                  active_project_id:
                    type: string
                    nullable: true
                    description: The pinned project, or null when the key is un-pinned.
        '401':
