...
> Switch the API key context to a different company, issuing a new agent + key.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/select-company
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/auth/select-company:
    post:
      tags:
        - Auth & Onboarding
      summary: Switch company context
      description: >-
        Switch the API key context to a different company, issuing a new agent +
        key.
      requestBody:
        required: true
        content:
      ...
      responses:
        ...
        '403':
          description: >-
            forbidden. Either no user of the caller's organization is a member
            of the target one, or — `details.reason =
            "seal_not_portable_across_companies"` — the calling key is sealed to
            a governed subject, and tenant users are per-organization so that
            seal has no counterpart in the target. Select the organization from
            the dashboard, or call this with the workspace key.
          content:
            application/json:
              schema:
