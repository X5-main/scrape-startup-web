...
# Retired verification-code endpoint

> This legacy endpoint is retired and always returns 400. Use the durable browser magic-link flow via `naive auth email <address>`.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/verify
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/verify:
    post:
      tags:
        - Auth & Onboarding
      summary: Retired verification-code endpoint
      description: >-
        This legacy endpoint is retired and always returns 400. Use the durable
        browser magic-link flow via `naive auth email <address>`.
      responses:
        '400':
