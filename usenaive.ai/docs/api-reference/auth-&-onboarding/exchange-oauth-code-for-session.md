...
> Exchange a one-time OAuth code for a session cookie and user/company context.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/auth/session/exchange
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/auth/session/exchange:
    post:
      ...
      description: >-
        Exchange a one-time OAuth code for a session cookie and user/company
        context.
      requestBody:
        required: true
        content:
      ...
      responses:
        '200':
          description: Session established (sets `naive_session` cookie).
          content:
            application/json:
              schema:
                type: object
