...
> Costs 2.5 credits, plus 0.5 credits when it targets X, or 5 credits instead when that X post contains a link.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/social/posts/{id}/publish
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/social/posts/{id}/publish:
    post:
      ...
      description: >-
        Costs 2.5 credits, plus 0.5 credits when it targets X, or 5 credits
        instead when that X post contains a link.
      parameters:
        - name: id
          in: path
          required: true
