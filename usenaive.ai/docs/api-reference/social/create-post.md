...
> Create a social post; saved as a draft unless `publish_now` or `scheduled_at` is set. Publishing (immediately or scheduled) costs 2.5 credits, plus 0.5 credits when it targets X, or 5 credits instead when that X post contains a link. Drafts are free.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/social/posts
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/social/posts:
    post:
      ...
      summary: Create post
      description: >-
        Create a social post; saved as a draft unless `publish_now` or
        `scheduled_at` is set. Publishing (immediately or scheduled) costs 2.5
        credits, plus 0.5 credits when it targets X, or 5 credits instead when
        that X post contains a link. Drafts are free.
      requestBody:
        required: true
        content:
          application/json:
