...
> REAL, and it corrects its own name on the wire. This set is a DEFAULT, not a statute: a kit can opt out of any member with `requiresApproval: false`, so the response carries `waivable: true` and says why. Calling it a statute without that would overstate what cannot be waived.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/policy/statute
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/policy/statute:
    get:
      tags:
        - Governance
      summary: The actions that default to human approval
      description: >-
        REAL, and it corrects its own name on the wire. This set is a DEFAULT,
        not a statute: a kit can opt out of any member with `requiresApproval:
        false`, so the response carries `waivable: true` and says why. Calling
        it a statute without that would overstate what cannot be waived.
      responses:
        '200':
          description: The default approval set.
          content:
            application/json:
              schema:
                type: object
