...
> 501: no grants table. A Grant's five required properties are five fields precisely so they can be falsified; a grant surface that cannot report them cannot be checked by anyone.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/grants
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/grants:
    get:
      ...
      summary: List grants
      description: >-
        501: no grants table. A Grant's five required properties are five fields
        precisely so they can be falsified; a grant surface that cannot report
