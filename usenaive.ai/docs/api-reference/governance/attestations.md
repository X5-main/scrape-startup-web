...
> 501: no attestations table. The nearest thing is `tasks_mirror.verification_evidence`, a single JSONB blob on a task row, which cannot answer pass/fail/absent per required claim.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/attestations
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/attestations:
    get:
      ...
      description: >-
        501: no attestations table. The nearest thing is
        `tasks_mirror.verification_evidence`, a single JSONB blob on a task row,
        which cannot answer pass/fail/absent per required claim.
      parameters:
        - name: cursor
          in: query
          required: false
