...
# The resolved authority for a tenant

> REAL. The AccountKit as the gate sees it, plus which of C-23's six policy layers this build actually represents and why the other three do not.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/policy/snapshot
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/policy/snapshot:
    get:
      tags:
        - Governance
      summary: The resolved authority for a tenant
      description: >-
        REAL. The AccountKit as the gate sees it, plus which of C-23's six
        policy layers this build actually represents and why the other three do
        not.
      parameters:
        - name: tenant
          in: query
