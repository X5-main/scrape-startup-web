...
# The resolved connection policy

> REAL, and the most consequential read on this surface. The connection family is the widest governed lane an agent reaches and BOTH kit-creation paths hardcode `mode: "open"`. `wide: true` is its own boolean rather than an inference from an empty toolkit list, because under `open` an empty list means ALL toolkits are reachable — the opposite of what an empty list usually reads as.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/connections/policy
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/connections/policy:
    get:
      tags:
        - Governance
      summary: The resolved connection policy
      description: >-
        REAL, and the most consequential read on this surface. The connection
        family is the widest governed lane an agent reaches and BOTH
        kit-creation paths hardcode `mode: "open"`. `wide: true` is its own
        boolean rather than an inference from an empty toolkit list, because
        under `open` an empty list means ALL toolkits are reachable — the
        opposite of what an empty list usually reads as.
      parameters:
        - name: tenant
          in: query
          required: false
              ...
              schema:
                ...
                properties:
                  ...
                  wide:
                    type: boolean
                    description: >-
                      TRUE when the policy resolves to `open` — every toolkit
                      and every tool is reachable.
                  wide_because:
                    type: string
                    nullable: true
                  toolkits:
