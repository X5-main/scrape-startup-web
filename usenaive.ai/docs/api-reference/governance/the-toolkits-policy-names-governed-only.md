...
# The toolkits policy names (governed only)

> Answers from the POLICY, never from the provider catalogue. Under `open`, policy names nothing and `items` is null with `wide: true` — which is the honest answer to `which toolkits does policy name`, rather than a listing of ~1,000 third-party toolkits under a policy path.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/connections/toolkits
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/connections/toolkits:
    get:
      tags:
        - Governance
      summary: The toolkits policy names (governed only)
      description: >-
        Answers from the POLICY, never from the provider catalogue. Under
        `open`, policy names nothing and `items` is null with `wide: true` —
        which is the honest answer to `which toolkits does policy name`, rather
        than a listing of ~1,000 third-party toolkits under a policy path.
      parameters:
        - name: tenant
          in: query
          required: false
      ...
      responses:
        '200':
          description: The governed toolkit list.
          content:
            application/json:
              schema:
                type: object
