...
# The ceilings, and the four honesty facts

> REAL. Prints the two pagination ceilings that DO bound this surface, the tenant's budget, and the four mechanisms that claim a property the platform does not have — the idempotency store's three defects, read-permissive revocation, the wide-open connections mode, and unverified deprecation headers. `snapshot.notAfter` and `grant.maxUntil` are null because nothing configures or enforces them; a ceiling reported as a number nobody set is worse than one reported as absent.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/limits
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/limits:
    get:
      tags:
        - Governance
      summary: The ceilings, and the four honesty facts
      description: >-
        REAL. Prints the two pagination ceilings that DO bound this surface, the
        tenant's budget, and the four mechanisms that claim a property the
        platform does not have — the idempotency store's three defects,
        read-permissive revocation, the wide-open connections mode, and
        unverified deprecation headers. `snapshot.notAfter` and `grant.maxUntil`
        are null because nothing configures or enforces them; a ceiling reported
        as a number nobody set is worse than one reported as absent.
      parameters:
        - name: tenant
          in: query
          required: false
      ...
      responses:
        '200':
          description: Limits and honesty.
          content:
            application/json:
              schema:
                type: object
