...
> Create a new inbox address on an active domain. Also available as `POST /v1/email/create-inbox`.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/email/inboxes
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/email/inboxes:
    post:
      ...
      description: >-
        Create a new inbox address on an active domain. Also available as `POST
        /v1/email/create-inbox`.
      requestBody:
        required: true
        content:
          application/json:
