...
> Send an outbound email from an owned inbox. Costs 0.016 credits and is metered against the plan quota. Cancelling a scheduled send refunds the charge in full.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/email/send
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/email/send:
    post:
      ...
      summary: Send email
      description: >-
        Send an outbound email from an owned inbox. Costs 0.016 credits and is
        metered against the plan quota. Cancelling a scheduled send refunds the
        charge in full.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                from_inbox:
                  type: string
                  description: Inbox UUID or full email address.
                to:
                  type: string
                subject:
                  type: string
      ...
      content:
        application/json:
          schema:
            $ref: '#/components/schemas/Error'
    RateLimited:
      description: Rate or quota limit exceeded. Includes a `Retry-After` header.
      content:
        application/json:
          schema:
            $ref: '#/components/schemas/Error'
