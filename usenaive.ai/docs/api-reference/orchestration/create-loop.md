...
> Create a Loop: a recurring cron-source subscription bound to an agent run. Records the binding row and best-effort creates the backing Hermes cron job; if no runtime is reachable the binding still persists (`cron_job_id: null`) and is reconciled later.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/loops
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/loops:
    post:
      ...
      summary: Create loop
      description: >-
        Create a Loop: a recurring cron-source subscription bound to an agent
        run. Records the binding row and best-effort creates the backing Hermes
        cron job; if no runtime is reachable the binding still persists
        (`cron_job_id: null`) and is reconciled later.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                ...
                prompt:
                  type: string
                  description: Prompt the loop runs. Required.
                name:
                  type: string
                  nullable: true
                profileName:
                  type: string
                  description: >-
                    Agent profile to run as. Defaults to `ceo` when the backing
                    cron job is created.
                profile_name:
                  type: string
                  description: >-
                    Snake-case alias for `profileName`; used only when
                    `profileName` is absent.
      responses:
        '201':
          description: Created.
          content:
              ...
              schema:
                ...
                properties:
                  ...
                  cron_job_id:
                    ...
                    nullable: true
                    description: >-
                      Id of the backing Hermes cron job, or null when no runtime
                      was reachable.
        '400':
