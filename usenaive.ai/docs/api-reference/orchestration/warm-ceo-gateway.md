...
> Poll the sidecar until the Hermes gateway is ready (pre-warm before sending).



## OpenAPI

````yaml /api-reference/openapi.json post /v1/companies/{id}/ceo/warm
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/companies/{id}/ceo/warm:
    post:
      tags:
        - Orchestration
      summary: Warm CEO gateway
      description: >-
        Poll the sidecar until the Hermes gateway is ready (pre-warm before
        sending).
      parameters:
        - name: id
          in: path
