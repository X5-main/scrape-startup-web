...
> Same as `GET /v1/companies/{id}/ceo/status`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/users/{user_id}/ceo/status
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/ceo/status:
    get:
      tags:
        - Orchestration
      summary: CEO status (per-user)
      description: >-
        Same as `GET /v1/companies/{id}/ceo/status`, scoped through the tenant
        user route.
      parameters:
