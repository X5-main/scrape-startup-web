...
# Stream CEO run (SSE) (per-user)

> Same as `GET /v1/companies/{id}/ceo/runs/{runId}/stream`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/users/{user_id}/ceo/runs/{runId}/stream
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/ceo/runs/{runId}/stream:
    get:
      tags:
        - Orchestration
      summary: Stream CEO run (SSE) (per-user)
      description: >-
        Same as `GET /v1/companies/{id}/ceo/runs/{runId}/stream`, scoped through
        the tenant user route.
      parameters:
