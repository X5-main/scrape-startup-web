...
> Patch a Loop's binding. `prompt`/`profileName` are merged into the subscription's `target`. Setting `active:false` pauses the loop (the backing cron job is deleted and `cronJobId` cleared); re-activating or changing schedule/prompt/profile recreates it.



## OpenAPI

````yaml /api-reference/openapi.json patch /v1/loops/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/loops/{id}:
    patch:
      ...
      summary: Update loop
      description: >-
        Patch a Loop's binding. `prompt`/`profileName` are merged into the
        subscription's `target`. Setting `active:false` pauses the loop (the
        backing cron job is deleted and `cronJobId` cleared); re-activating or
        changing schedule/prompt/profile recreates it.
      parameters:
        - name: id
          in: path
          required: true
