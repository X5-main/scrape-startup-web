...
> Trigger a Hermes CEO run. Also available at `/v1/companies/{id}/ceo/run` and `/v1/users/{user_id}/ceo/run`.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/companies/{id}/ceo/run
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/companies/{id}/ceo/run:
    post:
      tags:
        - Orchestration
      summary: Start CEO run
      description: >-
        Trigger a Hermes CEO run. Also available at `/v1/companies/{id}/ceo/run`
        and `/v1/users/{user_id}/ceo/run`.
      parameters:
        - name: id
          in: path
          required: true
          ...
          application/json:
            schema:
              ...
              properties:
                prompt:
                  type: string
                  description: User text (preferred).
                message:
                  type: string
      ...
      responses:
        '200':
          description: Run started (sidecar passthrough).
          content:
            application/json:
              schema:
                type: object
        ...
        '502':
          description: Container not running or not responding.
          content:
            application/json:
              schema:
