...
````yaml /api-reference/openapi.json get /v1/system/outbox
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/system/outbox:
    get:
      tags:
        - Orchestration
      summary: List sidecar outbox commands
      description: >-
        The dead-letter surface for agent-runtime writes. Returns the company's
        `sidecar_commands` rows, newest first. Defaults to the commands that
        have NOT landed (`pending,delivering,dead`). Requires an owner session
        cookie.
      parameters:
        - name: status
          in: query
          description: >-
            Comma-separated statuses to include. Unrecognised values are
            dropped; if nothing valid remains the request fails with
            `invalid_input`. Defaults to `pending,delivering,dead`.
          schema:
            type: string
            example: dead
        - name: limit
          in: query
          description: >-
            Max rows to return. Clamped to 200; non-numeric values fall back to
            50.
          schema:
            type: integer
            default: 50
            ...
            application/json:
              schema:
                ...
                properties:
                      ...
                      properties:
                        ...
                        path:
                          ...
                          description: >-
                            ...
                            /control/:companyId/:tenantUserId.
                        subject:
                          type: object
                          nullable: true
                          description: >-
                            What the command is about, or null when the row
                            carries no subject_type.
                          properties:
                            type:
                              type: string
                            id:
                  ...
                  count:
                    type: integer
                    description: Number of commands in this page.
                  dead_letters:
                    type: integer
                    description: How many of the returned commands are `dead`.
                  warning:
                    type: string
                    description: >-
                      Present only when `dead_letters` > 0: the work those
                      commands represent has not happened and will not retry on
                      its own.
        '400':
