...
> Returns the run plus every ledger event and artifact. A non-privileged agent key may read only its own runs; any other id 404s with the same shape as a missing run so existence never leaks.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/runs/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/runs/{id}:
    get:
      tags:
        - Orchestration
      summary: Get an agent run with its full ledger
      description: >-
        Returns the run plus every ledger event and artifact. A non-privileged
        agent key may read only its own runs; any other id 404s with the same
        shape as a missing run so existence never leaks.
      parameters:
        - name: id
          in: path
          required: true
