...
> Request a memory write via the CEO run (the CEO incorporates it into MEMORY.md).



## OpenAPI

````yaml /api-reference/openapi.json post /v1/memory
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/memory:
    post:
      ...
      summary: Add memory
      description: >-
        Request a memory write via the CEO run (the CEO incorporates it into
        MEMORY.md).
      requestBody:
        required: true
        content:
