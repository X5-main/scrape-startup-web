...
# Resume cron job for a tenant user



## OpenAPI

````yaml /api-reference/openapi.json post /v1/users/{user_id}/cron/{id}/resume
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/cron/{id}/resume:
    post:
      tags:
        - Orchestration
      summary: Resume cron job for a tenant user
      parameters:
        - name: user_id
          in: path
          required: true
          schema:
            type: string
          description: Tenant id, or `default`/`me`.
        - name: id
          in: path
          required: true
          schema:
            type: string
          description: Cron job id (naive uuid or Hermes job id).
