...
> Inject text into the active CEO run.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/companies/{id}/ceo/message
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/companies/{id}/ceo/message:
    post:
      tags:
        - Orchestration
      summary: Send CEO message
      description: Inject text into the active CEO run.
      parameters:
        - name: id
          in: path
          required: true
