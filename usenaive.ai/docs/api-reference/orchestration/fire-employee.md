...
> Archive employee (soft delete) and deprovision from the container.



## OpenAPI

````yaml /api-reference/openapi.json delete /v1/employees/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/employees/{id}:
    delete:
      ...
      description: Archive employee (soft delete) and deprovision from the container.
      parameters:
        - name: id
          in: path
          required: true
