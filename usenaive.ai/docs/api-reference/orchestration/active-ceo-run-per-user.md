...
# Active CEO run (per-user)

> Same as `GET /v1/companies/{id}/ceo/runs/active`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/users/{user_id}/ceo/runs/active
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/ceo/runs/active:
    get:
      tags:
        - Orchestration
      summary: Active CEO run (per-user)
      description: >-
        Same as `GET /v1/companies/{id}/ceo/runs/active`, scoped through the
        tenant user route.
      parameters:
