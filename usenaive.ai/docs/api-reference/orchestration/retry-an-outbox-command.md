...
````yaml /api-reference/openapi.json post /v1/system/outbox/{id}/retry
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/system/outbox/{id}/retry:
    post:
      tags:
        - Orchestration
      summary: Retry an outbox command
      description: >-
        Resurrect a dead (or stuck) sidecar command after fixing the cause.
        Claims the row atomically and re-delivers it with a fresh attempt
        budget; a delivery failure is reported in the 200 body (`delivered:
        false`), not as an error status. Company-wide: an owner session may
        retry ANY command in the company. Takes no request body.
      parameters:
        - name: id
          in: path
          required: true
          description: The sidecar command id.
          schema:
            type: string
            format: uuid
      responses:
        '200':
          description: Retry attempted. Inspect `delivered`.
          content:
            application/json:
              schema:
                type: object
                properties:
                  delivered:
                    type: boolean
                    description: >-
                      True when the agent runtime acknowledged the command on
                      this attempt.
                  command_id:
                    type: string
                    format: uuid
                  error:
                    type: string
                    nullable: true
                    description: Delivery failure message, or null on success.
                  response:
                    type: object
                    nullable: true
                    additionalProperties: true
                    description: The raw sidecar ack, or null when delivery failed.
        '400':
          description: >-
            `invalid_input` — the command was already delivered, or it is
            currently being delivered by the drain (retry is not needed).
          content:
            application/json:
              schema:
