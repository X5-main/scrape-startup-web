...
# Get CEO run (per-user)

> Same as `GET /v1/companies/{id}/ceo/runs/{runId}`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/users/{user_id}/ceo/runs/{runId}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/ceo/runs/{runId}:
    get:
      tags:
        - Orchestration
      summary: Get CEO run (per-user)
      description: >-
        Same as `GET /v1/companies/{id}/ceo/runs/{runId}`, scoped through the
        tenant user route.
      parameters:
      ...
      responses:
        '200':
          description: OK (sidecar passthrough).
          content:
            application/json:
              schema:
                type: object
