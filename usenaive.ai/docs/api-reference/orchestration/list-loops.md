...
> List the company's Loops (kind=`loop` cron-source trigger subscriptions). Scoped to the resolved subject's tenant user when the request resolves one.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/loops
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/loops:
    get:
      ...
      summary: List loops
      description: >-
        List the company's Loops (kind=`loop` cron-source trigger
        subscriptions). Scoped to the resolved subject's tenant user when the
