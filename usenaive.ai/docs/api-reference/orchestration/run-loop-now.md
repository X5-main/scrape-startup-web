...
> Trigger the Loop immediately. Records a `trigger_deliveries` row, then triggers the backing cron job (or falls back to a hidden `/ceo/run`). Returns `ok:false` with `run_id:null` when the runtime was unreachable — the request still succeeds with 200.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/loops/{id}/run
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/loops/{id}/run:
    post:
      ...
      description: >-
        Trigger the Loop immediately. Records a `trigger_deliveries` row, then
        triggers the backing cron job (or falls back to a hidden `/ceo/run`).
        Returns `ok:false` with `run_id:null` when the runtime was unreachable —
