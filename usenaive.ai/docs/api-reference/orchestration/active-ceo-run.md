...
> Return the in-flight CEO run so a client can re-attach after reload.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/companies/{id}/ceo/runs/active
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/companies/{id}/ceo/runs/active:
    get:
      tags:
        - Orchestration
      summary: Active CEO run
      description: Return the in-flight CEO run so a client can re-attach after reload.
      parameters:
        - name: id
          in: path
          required: true
