...
# List CEO messages (per-user)

> Same as `GET /v1/companies/{id}/ceo/messages`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/users/{user_id}/ceo/messages
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/users/{user_id}/ceo/messages:
    get:
      tags:
        - Orchestration
      summary: List CEO messages (per-user)
      description: >-
        Same as `GET /v1/companies/{id}/ceo/messages`, scoped through the tenant
        user route.
      parameters:
