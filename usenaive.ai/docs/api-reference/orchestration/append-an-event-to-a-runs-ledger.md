...
# Append an event to a run's ledger

> Write-guarded: allowed for an owner/operator session, the config-root CEO, the sealed subject that owns the run, or the agent whose agent_id matches the run. Anything else 404s.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/runs/{id}/events
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/runs/{id}/events:
    post:
      tags:
        - Orchestration
      summary: Append an event to a run's ledger
      description: >-
        Write-guarded: allowed for an owner/operator session, the config-root
        CEO, the sealed subject that owns the run, or the agent whose agent_id
        matches the run. Anything else 404s.
      parameters:
        - name: id
          in: path
          required: true
            ...
            schema:
              ...
              properties:
                ...
                actor_type:
                  type: string
                  maxLength: 80
                  description: Defaults to the calling principal's actor type.
                actor_id:
                  type: string
                  maxLength: 255
                  description: Defaults to the calling principal's id.
                title:
                  type: string
                  maxLength: 255
                summary:
            ...
            application/json:
              schema:
                ...
                properties:
                  event:
                    type: object
                    description: Serialised run ledger event (formatRunEvent).
                    properties:
                      id:
                        type: string
                        format: uuid
                      ...
                      event_status:
                        ...
                        enum:
                          ...
                          - skipped
                        description: >-
                          Nullable; DB check allows NULL or one of the listed
                          values.
                      actor_type:
                        type: string
                        nullable: true
