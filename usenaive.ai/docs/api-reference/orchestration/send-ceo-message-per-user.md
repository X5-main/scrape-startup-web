...
# Send CEO message (per-user)

> Same as `POST /v1/companies/{id}/ceo/message`, scoped through the tenant user route.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/users/{user_id}/ceo/message
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/users/{user_id}/ceo/message:
    post:
      tags:
        - Orchestration
      summary: Send CEO message (per-user)
      description: >-
        Same as `POST /v1/companies/{id}/ceo/message`, scoped through the tenant
        user route.
      parameters:
