...
> Trigger a run for a specific non-CEO agent.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/companies/{id}/agents/{agentId}/run
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/companies/{id}/agents/{agentId}/run:
    post:
      tags:
        - Orchestration
      summary: Run agent
      description: Trigger a run for a specific non-CEO agent.
      parameters:
        - name: id
          in: path
          required: true
