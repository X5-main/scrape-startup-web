...
# Erase a data subject's records

> Honour a data-subject deletion request: deletes every stored batch row that mentions the subject. UNMETERED on purpose — charging for a deletion request would make a legal obligation a cost centre. UNGATED for the same reason: this is the only People route that answers when the AccountKit does NOT grant `people`, because the opt-in gate stops personal data being acquired and a deletion acquires nothing — a workspace that turned the primitive off still holds batch records and still owes the subject an erasure. A valid key is still required and a revoked agent profile is still refused, which is what the 403 below now means. Records held by the upstream provider are its own; the response says so.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/people/erasure
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/erasure:
    post:
      tags:
        - People
      summary: Erase a data subject's records
      description: >-
        Honour a data-subject deletion request: deletes every stored batch row
        that mentions the subject. UNMETERED on purpose — charging for a
        deletion request would make a legal obligation a cost centre. UNGATED
        for the same reason: this is the only People route that answers when the
        AccountKit does NOT grant `people`, because the opt-in gate stops
        personal data being acquired and a deletion acquires nothing — a
        workspace that turned the primitive off still holds batch records and
        still owes the subject an erasure. A valid key is still required and a
        revoked agent profile is still refused, which is what the 403 below now
        means. Records held by the upstream provider are its own; the response
        says so.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              type: object
              description: At least one identifier.
              properties:
                email:
                  type: string
                linkedin:
  ...
  schemas:
    ErasureResponse:
      ...
      properties:
        jobs_erased:
          type: number
          description: Stored batch rows that mentioned the subject and were deleted.
        upstream_notice:
          type: string
          description: What the caller must still do at the upstream provider.
    Error:
      type: object
      properties:
        error:
