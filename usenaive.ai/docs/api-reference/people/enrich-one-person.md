...
> Resolve one identity to a full professional record. Today `linkedin`, or `name` plus `company`, actually resolve; an identity the provider cannot resolve is refused with a hint naming what can. A no-match answers 404 carrying `credits_used: 0` — nothing is charged.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/people/enrich
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/enrich:
    post:
      tags:
        - People
      summary: Enrich one person
      description: >-
        Resolve one identity to a full professional record. Today `linkedin`, or
        `name` plus `company`, actually resolve; an identity the provider cannot
        resolve is refused with a hint naming what can. A no-match answers 404
        carrying `credits_used: 0` — nothing is charged.
      parameters:
              ...
              properties:
                identity:
                  ...
                  properties:
                    ...
                    name:
                      type: string
                      description: Needs company or location to disambiguate.
                    company:
                      type: string
                    location:
                      type: string
