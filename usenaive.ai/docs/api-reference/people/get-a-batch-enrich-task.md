...
# Get a batch enrich task

> Batch status and, once complete, the enriched records with the matched/unmatched/failed tally.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/people/tasks/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/tasks/{id}:
    get:
      tags:
        - People
      summary: Get a batch enrich task
      description: >-
        Batch status and, once complete, the enriched records with the
        matched/unmatched/failed tally.
      parameters:
        - name: id
          in: path
          required: true
          description: Batch task id returned by POST /tasks.
          schema:
            type: string
      responses:
        '200':
