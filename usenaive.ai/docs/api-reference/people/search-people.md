...
> Find people by filter. METERED PER RECORD RETURNED at the same rate as enrich, so `limit: 50` costs ~50x `limit: 1` and search is not a cheaper route to the same record. A search that matches nothing is free. Above 50 records use POST /v1/people/tasks. Requires the `people` primitive to be enabled on the AccountKit — it is opt-in and off by default.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/people/search
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/search:
    post:
      ...
      description: >-
        Find people by filter. METERED PER RECORD RETURNED at the same rate as
        enrich, so `limit: 50` costs ~50x `limit: 1` and search is not a cheaper
        route to the same record. A search that matches nothing is free. Above
        50 records use POST /v1/people/tasks. Requires the `people` primitive to
        be enabled on the AccountKit — it is opt-in and off by default.
      parameters:
          ...
          application/json:
            schema:
              ...
              properties:
                filters:
                  type: object
                  description: At least one key. Unknown keys are rejected, not ignored.
                  properties:
                    title:
                      type: string
                    company:
                ...
                include_contact:
                  type: boolean
                  default: false
                  description: Adds work email; doubles the per-record cost.
      responses:
        '200':
          description: OK.
          content:
            application/json:
              schema:
                ...
                properties:
                  ...
                  credits_used:
                    type: number
                    description: Charged on records actually returned.
                  credits_remaining:
                    type: number
                  email_verification:
                    type: object
                    description: >-
                      Deliverability of the returned email, when contact data
                      was requested.
...
components:
  schemas:
    Person:
      type: object
      description: >-
        The canonical People record, normalized across providers.
        `match_likelihood` and `phone` are on the contract but not populated —
        the current provider does not score matches, and person-level phone
        numbers are withheld by the B2B boundary.
      properties:
        id:
          type: string
          description: Provider record id, namespaced by provider.
        name:
          type: string
        title:
          type: string
        ...
        email:
          type: string
          description: Work email. Present only with include_contact.
        match_likelihood:
          type: number
          description: Provider match confidence 1-10. Not populated.
        phone:
          type: string
          description: Not populated — withheld by the B2B boundary.
        firmographics:
          type: object
          properties:
            company_name:
