...
# Create a batch enrich task

> Asynchronous batch enrich, up to PRIMITIVE_BATCH_MAX identities (1,000 by default). Worst-case credits are held at submit and settled against the records actually matched — unmatched rows are refunded, and unmatched is reported separately from failed so a provider outage is never reported as "nobody exists".



## OpenAPI

````yaml /api-reference/openapi.json post /v1/people/tasks
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/tasks:
    post:
      tags:
        - People
      summary: Create a batch enrich task
      description: >-
        Asynchronous batch enrich, up to PRIMITIVE_BATCH_MAX identities (1,000
        by default). Worst-case credits are held at submit and settled against
        the records actually matched — unmatched rows are refunded, and
        unmatched is reported separately from failed so a provider outage is
        never reported as "nobody exists".
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                identities:
                  ...
                  description: Each entry needs at least one identity key.
                include_contact:
                  type: boolean
                  default: false
      responses:
        '202':
          description: Accepted — poll the task endpoint.
          content:
            application/json:
              schema:
                type: object
