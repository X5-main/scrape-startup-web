...
# People acceptable-use terms

> The prohibitions a caller is bound by, as JSON, so an agent can read them without opening a web page. The product surface of ToS Section 18: B2B only, not a consumer report, and never for employment, credit, insurance, housing or other sensitive decisions. Unmetered. NOT behind the opt-in gate: this is the one People route an AccountKit that has not enabled `people` can read, because it is what you read in order to decide whether to enable it. Every other People route answers 403 subprocessor_consent_required until it is.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/people/terms
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/people/terms:
    get:
      tags:
        - People
      summary: People acceptable-use terms
      description: >-
        The prohibitions a caller is bound by, as JSON, so an agent can read
        them without opening a web page. The product surface of ToS Section 18:
        B2B only, not a consumer report, and never for employment, credit,
        insurance, housing or other sensitive decisions. Unmetered. NOT behind
        the opt-in gate: this is the one People route an AccountKit that has not
        enabled `people` can read, because it is what you read in order to
        decide whether to enable it. Every other People route answers 403
        subprocessor_consent_required until it is.
      responses:
        '200':
          description: OK.
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/PrimitiveTermsResponse'
components:
  schemas:
    PrimitiveTermsResponse:
      type: object
      description: The acceptable-use attestation for a primitive. Unmetered.
      properties:
        attestation:
          type: object
