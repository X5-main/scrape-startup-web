...
# List inbound hook endpoints

> Lists the workspace's inbound hook endpoints (cloud event gateway). Scoped to the caller's company, and further filtered to the resolved subject tenant user when one is resolved. Ordered by `createdAt` descending. Secrets are never returned here — only `secret_set`.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/hooks
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/hooks:
    get:
      tags:
        - Webhooks
      summary: List inbound hook endpoints
      description: >-
        Lists the workspace's inbound hook endpoints (cloud event gateway).
        Scoped to the caller's company, and further filtered to the resolved
        subject tenant user when one is resolved. Ordered by `createdAt`
        descending. Secrets are never returned here — only `secret_set`.
      responses:
        '200':
          description: OK.
          content:
                  ...
                  endpoints:
                    ...
                    items:
                      ...
                      properties:
                        ...
                        slug:
                          type: string
                          description: Public slug used in the inbound URL (`hk_<hex>`).
                        name:
                          type: string
                          nullable: true
                        source:
                        ...
                        url:
                          ...
                          description: >-
                            Public inbound URL,
                            `https://api.usenaive.ai/webhooks/hooks/{slug}` —
                            the path this deployment routes. `null` when the
                            deployment has no inbound address (see
                            `webhook_ingress` on GET /v1/agents/status).
                        secret_set:
                          type: boolean
                          description: >-
                            Whether a signing secret exists. The secret itself
                            is never listed.
                  count:
                    type: integer
        '401':
