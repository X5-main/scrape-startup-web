...
> Generates a new signing secret for the endpoint and returns it once. The previous secret stops verifying immediately. Scoped to the caller's company.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/hooks/{id}/rotate-secret
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
tags:
  ...
  - name: Cron
    description: >-
      ...
      customers.
paths:
  /v1/hooks/{id}/rotate-secret:
    post:
      tags:
        - Webhooks
      summary: Rotate an inbound hook endpoint's signing secret
      description: >-
        Generates a new signing secret for the endpoint and returns it once. The
        previous secret stops verifying immediately. Scoped to the caller's
        company.
      parameters:
        - name: id
          in: path
          required: true
          description: Hook endpoint id (uuid).
          schema:
            type: string
            format: uuid
      responses:
        '200':
          description: Rotated. The new secret is returned only in this response.
          content:
            application/json:
              schema:
                type: object
                ...
                properties:
                  ...
                  secret:
                    type: string
                    description: New HMAC signing secret (`hksec_...`).
        '401':
        ...
        '404':
          description: >-
            `resource_not_found` — no hook endpoint with that id in this
            company.
          content:
            application/json:
              schema:
