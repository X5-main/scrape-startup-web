...
# Delete an inbound hook endpoint

> Deletes the endpoint scoped to the caller's company. Idempotent — deleting a missing or foreign-company id still returns `{ "ok": true }`.



## OpenAPI

````yaml /api-reference/openapi.json delete /v1/hooks/{id}
openapi: 3.0.3
info:
  title: Naïve API
  version: 2.0.0
...
paths:
  /v1/hooks/{id}:
    delete:
      tags:
        - Webhooks
      summary: Delete an inbound hook endpoint
      description: >-
        Deletes the endpoint scoped to the caller's company. Idempotent —
        deleting a missing or foreign-company id still returns `{ "ok": true }`.
      parameters:
        - name: id
          in: path
          required: true
